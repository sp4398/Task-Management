...
setEventTitle(title);
  ...
  return (
    <div style={{width:'50%',maxWidth:'300px'}} className='todotask'>
        {
            todos.length>0 ? (
                todos.map(todo=>(
                 ...
                 <span style={{fontSize:'0.9rem'}}><i>Scheduled For {new Date(todo.stuff_on).toLocaleDateString()}</i></span>
                  ...
                  <div className='buttons'>
                      <button onClick={()=>deleteStuff(todo.todo_id)}  className='btn btn-success'><FaTrash/> Delete</button>&nbsp;&nbsp;
                      <button  value={todo.todo_id} onClick={()=>editTodo({...todo})} className='btn btn-success'><FaPen/> Edit</button>&nbsp;&nbsp;
                      <button onClick={()=>showRemainingTimes(todo.stuff_on,todo.title)} className='btn btn-success'><FaStopwatch/>Remaining Times</button>
                  </div>
